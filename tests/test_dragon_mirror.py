...
class DragonMirrorRulesTests(unittest.TestCase):
    ...
    def test_manual_mulligan_is_an_explicit_two_player_phase(self):
        ...
        self.play(game, card)
        pool_event = next(e for e in game.events if e["kind"] == "rewind_taverns_pool")
    ...
    def test_additional_rewind_cards_open_and_resolve(self):
        for card_id in ("TIME_000", "TIME_002", "TIME_018", "TIME_602", "TIME_033"):
    ...
    def test_muradin_equips_and_returns_hammer(self):
        ...
        self.assertEqual("TIME_209t", game.players[0].weapon.card_id)
    ...
    def test_garona_halves_odd_health_by_rounding_damage_up(self):
        game = self.game(293)
        game.players[1].hand.append(game._entity("TIME_875t"))
        game.players[1].health = 15
        game.players[1].armor = 8
        self.play(game, self.add_hand(game, "TIME_875"))
    ...
    def test_first_portal_summons_opponent_demon(self):
        ...
        token = next(m for m in game.players[1].board if m.card_id == "TIME_020t2t")
    ...
    def test_axe_of_cenarius_draws_portal_after_hero_minion_kill(self):
        game = self.game(311)
        player = game.players[0]
        # Axe does not create or insert a portal.  It may only take one of
        # the portals that is already in the player's deck, chosen at random.
        player.deck.extend(
            [
                game._entity("TIME_020t2", started_in_deck=True),
                game._entity("TIME_020t4", started_in_deck=True),
            ]
        )
        ...
        game._after_hero_attack(player, weapon, (1, target.entity_id), 3)
        self.assertEqual(1, len(player.hand))
        self.assertIn(player.hand[0].card_id, {"TIME_020t2", "TIME_020t4"})
        self.assertEqual(1, sum(card.card_id in {"TIME_020t2", "TIME_020t4"} for card in player.deck))

    def test_axe_of_cenarius_does_nothing_without_existing_portal(self):
        game = self.game(312)
        player = game.players[0]
        target = self.add_board(game, "CORE_CS2_065", 1)
        target.damage = target.max_health - 1
